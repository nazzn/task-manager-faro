        ...
        <ul class="space-y-2">
          <li v-for="item in items" :key="item.to">
            ...
            <NuxtLink
              ...
              :class="isActive(item.to) ? 'bg-[#1F2937] text-white ' : 'text-white '"
            ...
            >
              <div class="flex items-center">
                <div
                  ...
                  :class="
                    isActive(item.to) ? 'bg-[#1F2937]' : 'bg-[#111827] group-hover:bg-[#1F2937]'
                  "
                >
                  <img :src="`/icons/main/${item.icon}`" :alt="item.label" class="w-5 h-5" >
                </div>
            ...
            >
              <ul v-if="item.label === 'وظایف' && route.path === '/'" class="mt-2 space-y-1 pr-4">
...
/* project label */
const projectLabelState = useState<string>("projectLabel", () => "آوپارداز همراه دیجیتال");
