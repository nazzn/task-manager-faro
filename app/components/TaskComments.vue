...
<template>
  <div class="space-y-4" dir="rtl">
    ...
    <div v-else class="space-y-3 max-h-72 overflow-y-auto custom-scrollbar pl-1">
      ...
      >
        <!-- آواتار -->
        <div class="w-8 h-8 rounded-full bg-[#238A63]/20 flex items-center justify-center flex-shrink-0 mt-0.5">
        ...
        <div class="flex-1 min-w-0">
          ...
          >
            <svg class="w-4 h-4 text-[#238A63] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path d="M15.172 7l-2.828-2.828a4 4 0 00-5.656 0L4.93 6.828a4 4 0 000 5.656l4.242 4.242a4 4 0 005.656 0l2.828-2.828M9 15l6-6" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
    ...
    <div class="rounded-2xl border border-slate-200 overflow-hidden focus-within:border-[#238A63] focus-within:ring-2 focus-within:ring-[#238A63]/10 transition">
      ...
      >
        ...
        <span class="text-[10px] text-slate-400 flex-shrink-0">{{ formatFileSize(selectedFile.size) }}</span>
        <button @click="removeSelectedFile" class="text-slate-400 hover:text-red-400 transition">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path d="M6 18L18 6M6 6l12 12" stroke-width="2" stroke-linecap="round"/>
          </svg>
        </button>
...
const removeSelectedFile = () => { selectedFile.value = null }
