  ...
  >
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar">
        <form id="editTaskForm" class="space-y-8" @submit.prevent="handleSubmit">
          <!-- Task Title -->
          <div class="space-y-1">
            <input
          ...
          <div class="flex flex-wrap items-start gap-4">
            ...
            <span v-if="statusRestrictionMsg" class="text-xs text-amber-600 mt-1 block">{{
              statusRestrictionMsg
            }}</span>
          ...
          <!-- خطای عمومی -->
          <div v-if="errorMessage" class="text-sm text-red-600 bg-red-50 p-3 rounded-xl">
            {{ errorMessage }}
          </div>
...
const handleSubmit = async () => {
  ...
  taskForm.value.subtasks = taskForm.value.subtasks.filter((s) => s.title.trim());
